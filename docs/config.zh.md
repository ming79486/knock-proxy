...
`proxy` 模式运行本地 client，链路为 knock 通过 -> 防火墙临时放行 -> TCP 认证 -> 可选加密转发 -> revoke，适合生产默认部署。`direct` 模式用于一次性 knock 后让 SSH/RDP/MySQL 等真实客户端直接连接服务端公网端口，链路为 knock 通过 -> 防火墙临时放行 -> 第一次真实 TCP 连接 -> revoke；建议配合较短的 `allow_seconds`、`remove_after_first_connect: true` 和较小的 `max_connections_per_knock` 使用。
...
| 字段 | 默认值 | 说明 |
| --- | --- | --- |
| `frame` | `envelope-v2` | TCP 认证 wire format。`envelope-v2` 是 bucketized encrypted envelope；`frame-v1` 仅用于旧版本兼容。 |
| `hint_mode` | `route-hint` | Envelope v2 hint 模式：`route-hint` 或 `none`。route hint 可避免逐个尝试所有 secret，同时固定头仍保持随机化。 |
| `frame_size_buckets` | `[128, 192, 256, 384, 512]` | Envelope v2 bucket 大小，必须严格递增且位于 128 到 512 字节之间。 |
| `padding_policy` | `random-bucket` | Envelope v2 padding 策略：`random-bucket` 或 `none`。 |
| `fail_delay_jitter_min` | `20ms` | TCP 认证失败后关闭连接前的最小有界 jitter。两个 jitter 都设为 `0s` 可关闭。 |
| `fail_delay_jitter_max` | `80ms` | TCP 认证失败后关闭连接前的最大有界 jitter，应保持在 auth timeout 预算内。 |
| `drain_on_fail_bytes` | `0` | 认证失败时可选 drain 的字节数，默认关闭。 |
| `drain_on_fail_timeout` | `0s` | 认证失败时可选 drain deadline，默认关闭。 |
...
传输加密在 TCP 认证通过后才开始。默认 `auth.frame: envelope-v2` 下，认证元数据已经封在固定 bucket 大小的 AEAD envelope 内；`transport.encryption` 只覆盖后续被代理的业务流。
...
- `proxy` 是生产默认模式：knock 通过 -> 防火墙临时放行 -> TCP auth envelope -> 可选加密转发 -> revoke。TCP auth envelope v2 使用随机 prefix、可选 route hint、AEAD-sealed metadata 和 bucket padding。
...
UDP knock 使用 binary frame v1。它已经移除 JSON 明文字段，但仍有固定外部 binary header，因此应视为当前兼容格式，而不是最强的被动识别防护格式。

TCP auth 默认使用 `envelope-v2`：`prefix_random[24]`、可选 `route_hint[8]`，以及 padding 到配置 bucket 的 AEAD-sealed envelope。`frame-v1` 仍可用于旧部署兼容，但它有固定 binary header，不应作为公网高风险场景的首选格式。

接收端只接受明确配置的格式，并对不支持的版本或 flags 返回清晰校验错误，避免静默降级或语义歧义。TCP SYN knock 的兼容性由 `syn-knock` / `syn-seq-knock` HMAC purpose、受保护 TCP 端口、client ID 和 time-slot 布局共同定义。后续 UDP envelope 变更应引入新 version，并在迁移期间保留明确校验错误。

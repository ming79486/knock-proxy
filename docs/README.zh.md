...
`knock-proxy` 用端口敲门把公网 TCP 管理服务藏到防火墙后面。服务端默认 DROP 受保护端口；客户端先发送 knock，验证通过后服务端只对该来源 IP 打开短暂访问窗口，再完成 TCP 二次认证并转发到本机 upstream。
...
- HMAC-SHA256 二次认证
- UDP knock/TCP auth 使用 timestamp + nonce 防重放；udp-seq/udp-passive-seq 使用多包 nonce 序列；TCP SYN knock/tcp-syn-seq 使用 time-slot HMAC
...
Windows TCP-SYN knock 推荐使用 WinDivert（https://github.com/basil00/WinDivert/）。把 `WinDivert.dll` 放在 `knock-proxy.exe` 同目录或 `WinDivert/` 子目录，并以管理员权限运行。WinDivert 不可用时会回退到 Npcap `Packet.dll`。
